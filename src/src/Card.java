...
public class Card implements Serializable {
     ...
     * @return
     */
    public URL getImageFilePath() {
        String imagePath;

        if (Round.darkmode == false) {
            if (this.colorLight == null || this.typeLight == this.getTypeLight().WILD_DRAW_FOUR) {
                imagePath = RELATIVE_IMAGE_DIRECTORY + this.typeLight + ".jpg";
            } else {
                imagePath = RELATIVE_IMAGE_DIRECTORY + this.colorLight + "_" + this.typeLight + ".jpg";
            }
        } else {
            if (this.colorDark == null || this.typeDark == this.getTypeDark().DARK_WILD_CARD) {
                imagePath = RELATIVE_IMAGE_DIRECTORY + this.typeDark + ".jpg";
            } else {
                imagePath = RELATIVE_IMAGE_DIRECTORY + this.colorDark + "_" + this.typeDark + ".jpg";
            }
        }

        // Return the URL from the resources folder
        return getClass().getResource(imagePath);
    }

    /**
     * enum TypeLight : provides all the possible types for a Card when the Card is
     * being played on the light side
     */
    enum TypeLight {
        ONE(1),
        TWO(2),
    ...
    /**
     * enum TypeDark: provides all the possible types for a Card when the Card is
     * being played on the dark side
     */
    enum TypeDark {
        ONE(1),
        TWO(2),
    ...
    /**
     * Card : Constructor for the Class, takes in parameters to set values for both
     * light and dark side of the card
     * 
     * @param colorLight
     * @param typeLight
     * @param colorDark
    ...
    /**
     * getValue : return's the value depending on which side of the card the current
